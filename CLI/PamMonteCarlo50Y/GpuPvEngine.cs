...
/// <summary>
/// Host-side executor for the <see cref="McPvKernel"/>.
///
/// Converts PAM contracts to <see cref="McContractGpu"/> + <see cref="McPamEventGpu"/>
/// on the CPU, then uploads everything to device buffers and launches the kernel.
///
/// Accelerator fallback chain: CUDA → OpenCL → CPU (same policy as existing executors).
/// Buffer pooling: device buffers grow on demand and are never reallocated unless needed.
/// </summary>
public sealed class GpuPvEngine : IDisposable
{
    ...
    private Action<Index2D,
                    ArrayView1D<McContractGpu,    Stride1D.Dense>,
                    ArrayView1D<McPamEventGpu,    Stride1D.Dense>,
                    ArrayView1D<double,           Stride1D.Dense>,
                    ArrayView1D<double,           Stride1D.Dense>,
                    int, int, int,
                    ArrayView1D<McPvGpuResult,    Stride1D.Dense>>? _kernel;

    private MemoryBuffer1D<McContractGpu,    Stride1D.Dense>? _contractsBuf;
    private MemoryBuffer1D<McPamEventGpu,    Stride1D.Dense>? _eventsBuf;
    private MemoryBuffer1D<double,           Stride1D.Dense>? _dfBuf;
    private MemoryBuffer1D<double,           Stride1D.Dense>? _ratesBuf;
    private MemoryBuffer1D<McPvGpuResult,    Stride1D.Dense>? _resultsBuf;

    private long _contractsCap, _eventsCap, _dfCap, _ratesCap, _resultsCap;

    /// <summary>Name of the underlying ILGPU accelerator.</summary>
    public string AcceleratorName => _accelerator.Name;

    private GpuPvEngine(Context context, Accelerator accelerator)
    {
        _context     = context;
        _accelerator = accelerator;
    }

    /// <summary>Create engine using best available accelerator (CUDA → OpenCL → CPU).</summary>
    public static GpuPvEngine CreateDefault()
    {
        ...
        var eng = new GpuPvEngine(context, acc!);
        eng.EnsureKernel();
        return eng;
    }
    ...
    /// <summary>
    /// Evaluate a slice of the portfolio under a slice of scenarios.
    ///
    /// Returns a flat <see cref="McPvGpuResult"/> array with layout
    /// <c>results[c * numScenarios + s]</c>.
    /// </summary>
    public McPvGpuResult[] Evaluate(
        ...
        DateTime                        maturityHorizon)
    {
        if (contracts.Count == 0 || numScenarios == 0)
            return Array.Empty<McPvGpuResult>();
        ...
        // 6. D2H copy
        var results = new McPvGpuResult[outcomeCount];
    ...
    private static void BuildGpuArrays(
        ...
        long                            baseDateTicks,
        int                             numMonths,
        out McContractGpu[]             gpuContracts,
        out McPamEventGpu[]             gpuEvents)
    {
        gpuContracts = new McContractGpu[contracts.Count];
        var evList   = new List<McPamEventGpu>(contracts.Count * 20);
        ...
        for (int c = 0; c < contracts.Count; c++)
        {
            ...
            double initNotional = terms.InitialExchangeDate > terms.StatusDate
                ? 0.0
                : terms.RoleSign * terms.NotionalPrincipal;
            double initRate     = terms.InitialExchangeDate > terms.StatusDate
                ? 0.0
                : terms.NominalInterestRate;

            gpuContracts[c] = new McContractGpu
            {
    ...
    private void EnsureBuffers(long contracts, long events, long dfAndRates, long results)
    {
        if (contracts > _contractsCap)
        {
            ...
            _contractsBuf = _accelerator.Allocate1D<McContractGpu>(_contractsCap);
        }
        if (events > _eventsCap)
        {
            _eventsBuf?.Dispose();
            _eventsCap = Grow(_eventsCap, events);
            _eventsBuf = _accelerator.Allocate1D<McPamEventGpu>(_eventsCap);
        }
        ...
        if (results > _resultsCap)
        {
            ...
            _resultsBuf = _accelerator.Allocate1D<McPvGpuResult>(_resultsCap);
        }
    }
    ...
    private void EnsureKernel()
    {
        ...
        _kernel = _accelerator.LoadAutoGroupedStreamKernel<
            ...
            ArrayView1D<double,           Stride1D.Dense>,
            int, int, int,
            ArrayView1D<McPvGpuResult,    Stride1D.Dense>>(McPvKernel.Kernel);
    }
